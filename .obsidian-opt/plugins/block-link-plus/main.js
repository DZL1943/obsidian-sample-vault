...
var tooltipPlugin = import_view2.ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.view = view;
...
var portalTypeAnnotation = import_state3.Annotation.define();
var flowIDAnnotation = import_state3.Annotation.define();
var flowIDStateField = import_state3.StateField.define({
  create: () => void 0,
  update(value, tr) {
...
var flowTypeStateField = import_state3.StateField.define({
  create: (state) => "none",
  update(value, tr) {
...
// src/basics/codemirror/flowEditor.tsx
var toggleFlowEditor = import_state4.Annotation.define();
var cacheFlowEditorHeight = import_state4.Annotation.define();
var preloadFlowEditor = import_state4.EditorState.transactionFilter.of(
  (tr) => {
    const newTrans = [];
...
var flowEditorInfo = import_state4.StateField.define({
  create() {
    return [];
  },
  update(value, tr) {
    ...
    for (const match of str2.matchAll(/!!\[\[([^\]]+)\]\]/g)) {
      ...
      const info = {
        ...
        type: 1 /* Embed */,
        height: existingInfo ? cachedHeight && cachedHeight[0] == id && cachedHeight[1] !== 0 ? cachedHeight[1] : existingInfo.height : -1,
        expandedState: existingInfo ? (toggleAnnotation == null ? void 0 : toggleAnnotation[0]) == id ? reverseExpandedState(existingInfo.expandedState) : existingInfo.expandedState : 1 /* AutoOpen */
      };
      newValues.push(info);
    }
    for (const match of str2.matchAll(/(?<!!)!\[\[([^\]]+)\]\]/g)) {
...
var _EmbedLeafManager = class {
  ...
  async createEmbedLeaf(args) {
    const leaf = new import_obsidian7.WorkspaceLeaf(this.plugin.app);
    const embed = {
    ...
    const component = new import_obsidian7.MarkdownRenderChild(args.containerEl);
    component.load();
    component.register(() => {
...
var InlineEditEngine = class {
  ...
  debugSkip(embedEl, key, data) {
    ...
    if (previous && previous.key === key && now - previous.at < 2e3)
      return;
      ...
      const uninstallExecuteCommand = around(this.plugin.app.commands, {
        executeCommand: (old) => {
          ...
          return function(command, ...args) {
            ...
            const isEditorCommand = typeof (command == null ? void 0 : command.editorCallback) === "function" || typeof (command == null ? void 0 : command.editorCheckCallback) === "function" || typeof (command == null ? void 0 : command.id) === "string" && command.id.startsWith("editor:");
  ...
  getLivePreviewState(entry) {
    ...
    if (!cm)
      return "unknown";
    try {
      if (((_c2 = (_b2 = cm.state) == null ? void 0 : _b2.field) == null ? void 0 : _c2.call(_b2, import_obsidian8.editorLivePreviewField, false)) === true) {
        return "live";
      }
    } catch (e) {
    }
  ...
  async waitForEditorView(view, timeoutMs = 500) {
    var _a2;
  ...
  parseRangeEmbedForReading(embedEl, sourcePath) {
    ...
    if (!notePath) {
      ...
      if (current instanceof import_obsidian8.TFile) {
        file = current;
      }
    } else {
  ...
  resolveEmbedLineRanges(parsed, cm) {
    ...
    const clamp = (line) => Math.min(Math.max(1, line), doc.lines);
  ...
  prepareEmbedShell(embedEl) {
    ...
    const hostParent = contentEl != null ? contentEl : embedEl;
    ...
    try {
      ...
      if (cm) {
        ...
        try {
          ...
          (_b2 = embed.view.editor) == null ? void 0 : _b2.setCursor({ line: startLine, ch: 0 });
          (_c2 = embed.view.editor) == null ? void 0 : _c2.scrollIntoView({ from: { line: startLine, ch: 0 }, to: { line: startLine, ch: 0 } }, true);
        } catch (e) {
        }
      } else {
      ...
      try {
        (_f2 = (_e2 = (_d2 = hostView == null ? void 0 : hostView.editor) == null ? void 0 : _d2.cm) == null ? void 0 : _e2.requestMeasure) == null ? void 0 : _f2.call(_e2);
      } catch (e) {
      }
...
var LivePreviewRangeEmbedChild = class extends import_obsidian8.MarkdownRenderChild {
  constructor(args) {
    super(args.embedEl);
    this.renderSeq = 0;
    this.mounted = false;
  ...
  async render() {
    ...
    const contentEl = this.embedEl.querySelector(".markdown-embed-content");
    if (!contentEl) {
      this.scheduleRetry(50);
      return;
    }
    ...
    const clamp = (line) => Math.min(Math.max(1, line), Math.max(1, lines.length));
    ...
    const renderChild = new import_obsidian8.MarkdownRenderChild(previewSizer);
    this.addChild(renderChild);
    renderChild.load();
    this.renderChild = renderChild;
    this.retryCount = 0;
    try {
      await import_obsidian8.MarkdownRenderer.renderMarkdown(fragment, previewSizer, this.file.path, renderChild);
    } catch (error) {
      console.error("InlineEditEngine: failed to render live preview range embed", error);
    }
  }
};
var ReadingRangeEmbedChild = class extends import_obsidian8.MarkdownRenderChild {
  constructor(args) {
    super(args.embedEl);
    this.renderSeq = 0;
    this.mounted = false;
...
var BlockLinkPlusSettingsTab = class extends import_obsidian10.PluginSettingTab {
  ...
  display() {
    ...
    this.addHeading(i18n_default.settings.inlineEdit.title).setDesc(i18n_default.settings.inlineEdit.desc);
    this.addToggleSetting("inlineEditEnabled").setName(i18n_default.settings.inlineEdit.enable.name).setDesc(i18n_default.settings.inlineEdit.enable.desc);
    this.addToggleSetting("inlineEditFile").setName(i18n_default.settings.inlineEdit.file.name).setDesc(i18n_default.settings.inlineEdit.file.desc);
    this.addToggleSetting("inlineEditHeading").setName(i18n_default.settings.inlineEdit.heading.name).setDesc(i18n_default.settings.inlineEdit.heading.desc);
    this.addToggleSetting("inlineEditBlock").setName(i18n_default.settings.inlineEdit.block.name).setDesc(i18n_default.settings.inlineEdit.block.desc);
...
function createViewPlugin(rule = "(^| )\u02C5[a-zA-Z0-9_]+$", timePattern = "\\d{1,2}:\\d{1,2}") {
  let decorator = new import_view5.MatchDecorator({
    regexp: new RegExp(rule, "g"),
    decoration: (match) => {
      try {
        const timeSectionRegex = new RegExp(`^#{1,6}\\s+${timePattern}$`);
        if (timeSectionRegex.test(match[0])) {
          return import_view5.Decoration.mark({ class: "time-section-plain" });
        }
      } catch (e) {
        console.error("Invalid time pattern in ViewPlugin:", e);
      }
      return import_view5.Decoration.mark({ class: "small-font" });
    }
  });
...
// src/ui/WhatsNewModal.ts
var WhatsNewModal = class extends import_obsidian11.Modal {
  ...
  onOpen() {
    ...
    const items = this.currentVersion === "1.8.0" ? i18n_default.whatsNew.v1_8_0 : i18n_default.whatsNew.fallback;
    if (items.length) {
...
function copyToClipboard(app, settings, file, links, isEmbed, alias, isUrl = false) {
  ...
  if (isUrl && settings.enable_url_notification) {
    new import_obsidian12.Notice("Obsidian URI copied to clipboard");
  } else if (isEmbed && settings.enable_embed_notification) {
    new import_obsidian12.Notice("Block embed link copied to clipboard");
  } else if (!isEmbed && !isUrl && settings.enable_block_notification) {
    new import_obsidian12.Notice("Block link copied to clipboard");
  }
}
...
function handleMultiLineBlock(plugin, file, isEmbed, head_analysis, editor, fileCache, isUrl = false) {
  if (plugin.settings.mult_line_handle == 0 /* oneline */) {
    ...
    if (!result.ok) {
      new import_obsidian13.Notice(result.message, 1500);
      return;
    }
...
function handleMultiLineBlock2(plugin, file, isEmbed, head_analysis, editor, fileCache, isUrl = false) {
  if (plugin.settings.mult_line_handle == 0 /* oneline */) {
    ...
    if (!result.ok) {
      new import_obsidian14.Notice(result.message, 1500);
      return;
    }
    const link = result.link;
    const alias = calculateAlias(plugin.settings, link, false, isEmbed, isUrl, plugin.settings.alias_length, head_analysis);
...
function resolveTags(context) {
  ...
  let frontmatterTags = [];
  const fmConfig = (_f2 = (_e2 = config.filters) == null ? void 0 : _e2.tags) == null ? void 0 : _f2.from_frontmatter;
    ...
    dataviewQueryResults: {
      ...
      pages: (pages.values || pages).slice(0, 10).map((page) => {
        ...
        return {
          ...
          outlinks: (_g = (_f2 = page.file) == null ? void 0 : _f2.outlinks) == null ? void 0 : _g.slice(0, 5).map((link) => link.path)
        };
      })
    },
    extractedSections: {
  ...
  try {
    ...
    for (const page of pages) {
      if (page.file && page.file.path) {
        ...
        if (pageFile instanceof import_obsidian15.TFile) {
...
// src/main.ts
var BlockLinkPlus = class extends import_obsidian16.Plugin {
  constructor() {
    super(...arguments);
