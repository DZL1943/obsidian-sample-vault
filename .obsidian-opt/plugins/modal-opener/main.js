...
var _ModalWindow = class extends import_obsidian2.Modal {
  ...
  async onOpen() {
    ...
    this.scope.register([], "Escape", async (evt) => {
      var _a2;
      evt.preventDefault();
    ...
    if (this.plugin.settings.doubleClickToEdit) {
      this.containerEl.addEventListener("dblclick", (evt) => {
        if (this.modalLeafRef) {
          ...
          if (view instanceof import_obsidian2.MarkdownView && view.getMode() === "preview") {
            evt.preventDefault();
            evt.stopPropagation();
  ...
  close() {
    var _a;
    const smmView = (_a = this.app.workspace.getLeavesOfType("smm").first()) == null ? void 0 : _a.view;
    if (smmView) {
      this.app.commands.executeCommandById("simple-mind-map:save-smm-mindmap-and-update-image");
      setTimeout(() => {
        this.doClose();
      }, 500);
    } else {
  ...
  onClose() {
    ...
    if (this.plugin.settings.enableRefreshOnClose && (dataType == "canvas" || dataType == "mindmapview" || dataType == "smm")) {
      const targetPath = (_b = (_a = _ModalWindow.activeInstance) == null ? void 0 : _a.file) == null ? void 0 : _b.path;
      const backlinks = [];
      if (targetPath) {
      ...
      const prevFile = ((_c = this.prevActiveLeaf) == null ? void 0 : _c.view) instanceof import_obsidian2.MarkdownView ? this.prevActiveLeaf.view.file : null;
  ...
  handleInternalLinkClick(event) {
    ...
    if (!target.closest(".workspace-leaf-content") || target.closest(".view-header"))
      return;
    if (target.closest('.workspace-leaf-content[data-type="excalidraw"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="tldraw-view"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="canvas"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="excel-view"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="excel-pro-view"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="mindmapview"]'))
      return;
    if (target.closest('.workspace-leaf-content[data-type="smm"]'))
      return;
    let linkText = "";
    const activeView = (_a = this.modalLeafRef) == null ? void 0 : _a.view;
    if (activeView instanceof import_obsidian2.MarkdownView) {
    ...
    if (isCtrlClick) {
      if (this.isValidURL(linkText)) {
        event.preventDefault();
        event.stopImmediatePropagation();
        window.require("electron").shell.openExternal(linkText);
      } else {
    ...
    if ((_b = _ModalWindow.activeInstance) == null ? void 0 : _b.contentEl) {
      if (this.isValidURL(linkText)) {
        ...
        event.stopImmediatePropagation();
        this.link = linkText;
        (_c = _ModalWindow.activeInstance) == null ? void 0 : _c.contentEl.setAttribute("data-src", linkText);
        if (this.webviewPlugin && this.modalLeafRef) {
          this.clearAllButton((_d = _ModalWindow.activeInstance) == null ? void 0 : _d.contentEl);
          ...
          if (this.plugin.settings.viewOfDisplayButton === "both" || this.plugin.settings.viewOfDisplayButton === "link") {
            this.clearAllButton((_e = _ModalWindow.activeInstance) == null ? void 0 : _e.contentEl);
            this.addFloatingButton((_f = _ModalWindow.activeInstance) == null ? void 0 : _f.contentEl);
          }
        }
      } else {
        ...
        if (!file)
          return;
        if (fragment) {
          (_h = _ModalWindow.activeInstance) == null ? void 0 : _h.contentEl.setAttribute("data-src", `${file.path}#${fragment}`);
        } else {
          (_i = _ModalWindow.activeInstance) == null ? void 0 : _i.contentEl.setAttribute("data-src", `${file.path}`);
        }
  ...
  getFloatingUtilityContainer(container) {
    let wrapper = container.querySelector(".floating-utility-container");
    if (!wrapper) {
  ...
  toggleTableOfContents(buttonContainer, path, isHover = false) {
    let tocContainer = this.contentEl.querySelector(".modal-opener-toc-container");
    if (tocContainer) {
      if (!isHover) {
        tocContainer.remove();
      }
...
var ModalOpenerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  displayGeneralSettings(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl).setName(t("Double-click to enter editing mode")).setDesc(t("Use double-click to enter editing mode from preview mode.")).addToggle((toggle) => toggle.setValue(this.plugin.settings.doubleClickToEdit).onChange(async (value) => {
      this.plugin.settings.doubleClickToEdit = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian3.Setting(containerEl).setName(t("Refresh view on close")).setDesc(t("Refresh embedded file thumbnails when closing modal windows, currently only refreshing after editing Canvas\u3001SMM and Markmind file")).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableRefreshOnClose).onChange(async (value) => {
  ...
  displayStyleSettings(containerEl) {
    ...
    new import_obsidian3.Setting(containerEl).setName(t("Enable animation and blur")).setDesc(t("Toggle to enable or disable animation and blur effects")).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableBlur).onChange(async (value) => {
  ...
  displayMenuSettings(containerEl) {
    ...
    if (this.plugin.settings.showCommandsContainer) {
      ...
      if (basesPlugin) {
        new import_obsidian3.Setting(commandsContainer).setClass("command-toggle-item").setName("Bases").addToggle((toggle) => toggle.setValue(this.plugin.settings.enabledCommands.bases).onChange(async (value) => {
...
var _ModalOpenerPlugin = class extends import_obsidian4.Plugin {
        ...
        if (target.classList.contains("cm-underline") || target.classList.contains("cm-url") || target.classList.contains("cm-link")) {
          ...
          if (activeView && activeView.editor) {
            ...
            if (linkMatch && this.isValidURL(linkMatch)) {
              ...
              if (openExternal) {
                window.require("electron").shell.openExternal(linkMatch);
              } else {
                const leaf = this.app.workspace.getLeaf(true);
  ...
  registerAltClickHandler() {
    this.altClickHandler = (evt) => {
      ...
      if (target.matches(".multi-select-pill-content > span")) {
        ...
        const frontmatterLinks = fileCache == null ? void 0 : fileCache.frontmatterLinks;
  ...
  handlePreviewModeLink(evt, isAltClick) {
    ...
    if (target.closest(".metadata-link")) {
      const inputElement = (_b = (_a = target.closest(".metadata-link")) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.querySelector(".metadata-input-longtext");
      const textContent = inputElement == null ? void 0 : inputElement.textContent;
      if (textContent) {
        link = textContent.replace(/^\[\[(.*?)\]\]$/, "$1").split("|")[0].trim() || "";
      }
    }
    if (target.closest(".outgoing-link-item")) {
      const treeItemIcon = (_c = target.closest(".outgoing-link-item")) == null ? void 0 : _c.querySelector(".tree-item-icon");
      const subtext = ((_f = (_e = (_d = target.closest(".outgoing-link-item")) == null ? void 0 : _d.querySelector(".tree-item-inner-subtext")) == null ? void 0 : _e.textContent) == null ? void 0 : _f.trim()) || "";
      const text = ((_i = (_h = (_g = target.closest(".outgoing-link-item")) == null ? void 0 : _g.querySelector(".tree-item-inner-text")) == null ? void 0 : _h.textContent) == null ? void 0 : _i.trim()) || "";
      if (subtext) {
    ...
    if (target.closest(".bases-cards-item") || target.closest(".bases-cards-cover")) {
      const cardItem = target.closest(".bases-cards-item");
      const cardLine = cardItem == null ? void 0 : cardItem.querySelector(".bases-cards-line");
      if (cardLine) {
        link = ((_j = cardLine.textContent) == null ? void 0 : _j.trim()) || "";
      }
    }
    const singleClickType = !import_obsidian4.Platform.isMobile ? this.settings.typeOfClickTrigger : this.settings.typeOfClickTriggerOnMobile;
    if (!isAltClick) {
  ...
  getPreviewModeLinkText(target) {
    ...
    if ((_c = target.textContent) == null ? void 0 : _c.includes("\u8BE5\u6587\u4EF6\u4E2D\u6CA1\u6709\u53EF\u9884\u89C8\u56FE\u7247")) {
  ...
  addCreateFileMenuItem(menu) {
    menu.addItem((item) => {
      item.setTitle(t("Create and edit in modal")).setIcon("file-plus").setSection("selection");
      ...
      if (basesPlugin && this.settings.enabledCommands.bases) {
        group1Count++;
        subMenu.addItem(
          (subItem) => subItem.setTitle("Bases").setIcon("table").onClick(() => {
      ...
      if (simpleMindMapPlugin && this.settings.enabledCommands.simplemindmap) {
        subMenu.addItem(
          (subItem) => subItem.setTitle("Simple Mind Map").setIcon("brain-circuit").onClick(async () => {
